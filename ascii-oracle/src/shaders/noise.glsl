// Simplex 3D noise
// Based on Ian McEwan's GLSL implementation
...
// Flow field - returns direction vector
vec2 flowField(vec2 p, float time) {
  float angle = snoise(vec3(p * 2.0, time * 0.1)) * 3.14159 * 2.0;
  return vec2(cos(angle), sin(angle));
}

// Curl noise for more interesting flow
