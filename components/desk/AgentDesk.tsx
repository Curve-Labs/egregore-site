...
function liveExecutors(workers: Worker[]): Array<"claude" | "codex"> {
  const cutoff = Date.now() - 90_000;
  return (["claude", "codex"] as const).filter((executor) => workers.some((worker) => {
...
function CreatePanel({ org, onClose, onCreated, token, workers }: {
  org: string; token: string | null; workers: Worker[]; onClose: () => void; onCreated: (task: Task) => void;
}) {
      ...
      <aside className="desk-create-panel" aria-label="Create task">
        ...
        <form onSubmit={submit}>
          ...
          <div className="desk-form-row">
            ...
            <label>Executor<select value={form.executor} onChange={(e) => setForm({ ...form, executor: e.target.value as "" | "codex" | "claude" })}><option value="">Auto-select live worker</option>{executors.map((executor) => <option key={executor} value={executor}>{executor === "claude" ? "Claude" : "Codex"} · live</option>)}</select></label>
          </div>
          <div className="desk-form-row">
            <label>Repository<input required value={form.repository} onChange={(e) => setForm({ ...form, repository: e.target.value })} placeholder="Curve-Labs/repo" /></label>
...
function Detail({ detail, token, workers, onChanged }: { detail: TaskDetail; token: string | null; workers: Worker[]; onChanged: () => void }) {
  const [action, setAction] = useState("");
  const [error, setError] = useState("");
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const executors = liveExecutors(workers);
  const [approvalExecutor, setApprovalExecutor] = useState<"claude" | "codex" | "">("");
  ...
  useEffect(() => {
    const preferred = detail.executor_preference;
    setApprovalExecutor(preferred && executors.includes(preferred) ? preferred : executors[0] || "");
  }, [detail.id, detail.executor_preference, workers]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <article className="desk-detail">
      ...
      <section className="desk-section desk-run-grid">
        <div><span className="desk-kicker">Current run</span><h2>{currentRun ? `${currentRun.phase} · ${currentRun.status}` : detail.status === "queued" ? `Waiting for ${detail.executor_preference || "an executor"}` : "Waiting for a worker"}</h2>{currentRun && <dl><div><dt>Worker</dt><dd>{currentRun.worker_id || "unassigned"}</dd></div><div><dt>Model</dt><dd>{currentRun.model || "—"}</dd></div><div><dt>Lane</dt><dd>{currentRun.lane}</dd></div></dl>}</div>
  ...
  return (
    <div className="desk-shell">
      ...
      <div className="desk-workspace">
        ...
        <main className="desk-content">{error && <div className="desk-banner" role="alert">{error}<button onClick={() => setError("")}>Dismiss</button></div>}{loading && !detail ? <div className="desk-loading">Loading work…</div> : !tasks.length ? <EmptyDesk onCreate={() => setCreating(true)} /> : detail ? <Detail detail={detail} token={token} workers={workers} onChanged={() => refresh()} /> : <div className="desk-loading">Loading task…</div>}</main>
      </div>
      {creating && <CreatePanel org={org} token={token} workers={workers} onClose={() => setCreating(false)} onCreated={handleCreated} />}
